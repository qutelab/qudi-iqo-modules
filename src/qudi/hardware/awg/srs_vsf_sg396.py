...
"""
This file contains the Qudi hardware module for SRS SG396 Series.

Copyright (c) 2021, the qudi developers. See the AUTHORS.md file at the top-level directory of this
distribution and on <https://github.com/Ulm-IQO/qudi-iqo-modules/>
Created by Cody Stewart (cody.stewart1@ucalgary.ca)
...
class SG396(PulserInterface):
    """ A hardware module for the Stanford Research Systems SG396 vector signal
        generator.

    Example config for copy-paste:

    pulser_SG396:
        module.Class: 'awg.srs_vsf_SG396.SG396'
        options:
            awg_visa_address: 'TCPIP::0.0.0.0::INSTR'
            awg_ip_address: '0.0.0.0'
            awg_ip_port: '5025'
            timeout: 60
    ...
    # allowable unit types for device commands
    __units_amplitude = {'dBm','rms','Vpp'}
    __units_frequency = {'GHz','MHz','kHz','Hz'}
    __units_time = {'ns','us','ms','s'}
    ...
    def on_activate(self):
        ...
        self.__max_seq_steps = 10 #int(self.query('SLIS:SEQ:STEP:MAX?'))
        self.__max_seq_repetitions = 1 #int(self.query('SLIS:SEQ:STEP:RCO:MAX?'))
        self.__min_waveform_length = 1 #int(self.query('WLIS:WAV:LMIN?'))
        self.__max_waveform_length = 1 #int(self.query('WLIS:WAV:LMAX?'))
    ...
    def _has_sequence_mode(self):
        if self.awg_model in ['AWG70001A', 'AWG70002A','SG396']:
